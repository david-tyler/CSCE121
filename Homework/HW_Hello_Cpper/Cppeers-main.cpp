...
void processPostsWithHashtags(Network& cppeers) {
  ...
  cout << "Enter tagname: ";
  cin >> tag;
  vector <Post*> userTags = cppeers.getPostsWithTag(tag);
  for(int i = 0; i < userTags.size(); i++)
  {
    cout << userTags.at(i)->getPostText() << endl;
  }
}

void processMostPopularHashtag(Network& cppeers) {
  // TODO(student): implement
  vector <string> pop_tags = cppeers.getMostPopularHashtag();
  for(int i = 0; i < pop_tags.size(); i++)
  {
    cout << pop_tags.at(i) << endl;
  }
}
