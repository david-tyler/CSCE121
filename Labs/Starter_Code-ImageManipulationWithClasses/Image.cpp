...
Image::Image(const Image& objectToCopy)
{
  width = objectToCopy.width;
  height = objectToCopy.height;
  ...
  for (unsigned int row=0; row<height; ++row) {
    for (unsigned int col=0; col<width; ++col) {
      image[col][row] = {objectToCopy[col][row].getRed(), objectToCopy[col][row].getGreen(),objectToCopy[col][row].getBlue()};
    }
  }
  

}
Image& Image::operator=(const Image& objectToCopy)
{
